...
予備領域として `10.30.3.32` 以降、`10.30.4.0/24` から `10.30.7.0/24` を残しています。
...
Public IP は付けません。`snet-admin` に静的 Private IP `10.30.3.10` を割り当て、Hub 側 Azure Bastion からの SSH だけを NSG で許可します。

実行前に以下を自分の環境に合わせて設定してください。

```hcl
admin_ssh_public_key              = "<SSH公開鍵>"
hub_azure_bastion_subnet_prefix   = "<Hub側AzureBastionSubnetのCIDR>"
```
