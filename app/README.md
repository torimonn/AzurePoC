...
生成するExcelブックは、内容を上から順に確認できる単一の `事業性評価シート` です。評価領域ごとにシートを分割しません。

| シート内の領域 | 主な内容 |
|---|---|
| 処理情報・入力資料 | Document IntelligenceとAzure OpenAIの利用情報、元資料、確認状態 |
| 評価概要 | 5領域の概要 |
...
│  ├─ excel_service.py    # 単一シートの事業性評価Excel生成
